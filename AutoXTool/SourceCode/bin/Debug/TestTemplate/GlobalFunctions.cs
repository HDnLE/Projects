	...
	{		
		...
		public static string RunWinApplication(string appName)
		{
			...
			while (caching)
			{
				...
				try
				{
					Host.Local.FindSingle<Ranorex.Form>("/form[@processname='Systemx']", 15000);
					try
					{
            ...
            try
            {
				if (objType == "RawText")
				{
					if (direction != "")
					{
						...
						else if (direction.Contains("datarow-"))
						{
							string rawText = direction.Replace("datarow-","");
							try
							{
								templateText = GetTemplateText(objPath, "@rawtext=");
							}
						...
						{
							string rawText = direction.Replace("hastext-","");
							templateText = GetTemplateText(objPath, "@rawtext~");
							objPath = objPath.Replace(templateText, rawText);
						}
						else
		...
		public static string RestartApplication()
		{
			...
			try
			{
				Ranorex.Form frmStartmeny = Host.Local.FindSingle<Ranorex.Form>("/form[@title~'Startmeny' and @processname='Systemx']", 15000);
			...
			string strUsername = details[0];
			string strPassword = details[1];
			...
			try
			{
				...
				Ranorex.Text txtUsername = Host.Local.FindSingle<Ranorex.Text>("/form[@title~'Passord' and @processname='Systemx']/text[@instance='0']", 5000);
				Ranorex.Text txtPassword = Host.Local.FindSingle<Ranorex.Text>("/form[@title~'Passord' and @processname='Systemx']/text[@instance='1']", 5000);
				txtUsername.Focus();
		...
		public static string RestartSystemX(bool autoLogin=true)
		{
			...
			try
			{
				...
				try
				{
					...
					try
					{
						Ranorex.Form frmProgFail = Host.Local.FindSingle<Ranorex.Form>("/form[@title='Bekreft']", 15000);
					...
					if (autoLogin)
					{
						try
						{
							...
							try
							{
								Ranorex.Form frmPassword = Host.Local.FindSingle<Ranorex.Form>("/form[@title~'Passord' and @processname='Systemx']", 15000);
								frmPassword.Activate();
								//frmPassword.PressKeys(username + "{TAB}" + password);
								...
								try		//Previous System X has calendar window after login
								{
									...
									try
									{
										Ranorex.Form frmBeskjedmeny = Host.Local.FindSingle<Ranorex.Form>("/form[@controlname='fmerk']", 2000);
										frmBeskjedmeny.Activate();
										frmBeskjedmeny.PressKeys("{ESCAPE}");	
									}
									catch (ElementNotFoundException)
									{
										//Do nothing
								...
								catch (ElementNotFoundException)
								{
									...
									/*try
									{
										Ranorex.Form frmPassword = Host.Local.FindSingle<Ranorex.Form>("/form[@controlname='FPassord']", 5000);
										frmPassword.Activate();
										frmPassword.PressKeys("{ENTER}");	
									}
									catch (ElementNotFoundException)
									{
										//Do nothing
